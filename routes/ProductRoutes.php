...
class ProductRoutes
{
  ...
  public static function setup($app, $database)
  {
    ...
    $app->group('/api/v1/products', function ($group) use ($productController) {
      ...
      /**
              Mendefinisikan rute DELETE untuk menghapus data produk.
              URL     : /api/v1/products/destroy/{id}
             Metode   : POST
             Aksi     : Memanggil fungsi destroy() di ProductController.
       */
      $group->delete('/destroy/{id}', [$productController, 'destroy']);
    });
  }
}
